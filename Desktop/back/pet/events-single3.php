
<?php include('header.php'); ?>

    <!-- About Section -->
    <section id="about" style="background-color:darkcyan">
        <div class="container" style="background-color:mediumseagreen">
            <div class="row" style="background-color: darkslategray;">
                <div class="col-lg-12 text-center">
                    <h2 class="section-heading" style="margin-top: 20px; margin-bottom: 15px; font-size: 40px;">Dog Race</h2>
                    <h3 class="section-subheading text-muted" style="margin-bottom:30px;">
					Have a nice time watching this video !</h3>
					<hr> </hr>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
					<center>
						<video width="70%" height="10%" controls>
						  <source src="videos/4.mp4" type="video/mp4">
						</video>   
					</center>
				</div>
            </div>
			<div style="color:rgb(199, 172, 172)">
				<div class="row" style="padding-top:60px">
					<div class="col-lg-offset-1 col-lg-10">
						<!-- <p>Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum </p>  -->
						<p style="color:cornsilk;">We hope you liked this amazing video. Dog Shows are generally among those shows who have been most popular among the mob in the past. present and in future. It not only gives pet owners a medium to entertain themselves by watching their dogs but is also a treat to the eyes of whole public watching it. Thess type of shows also help in generating a self esteem and confidence within the dogs. Not only this, it makes the world a happy place to live in. Dogs in various attires wearing different attitudes walk on the ramp and attract the judges an people around them. Their way, their gait their charisma all are factors judges are interested in. Dogs do find their joys in this out-of-daily routine show.</p>
					</div>
				</div>
				<div class="row" style="padding-top:20px; color:black;">
					<div class="col-lg-offset-1 col-lg-5">
						<!-- Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum Lorem Ipsum  -->
						We hope you liked this amazing video. Dog Shows are generally among those shows who have been most popular among the mob in the past. present and in future. It not only gives pet owners a medium to entertain themselves by watching their dogs but is also a treat to the eyes of whole public watching it. Thess type of shows also help in generating a self esteem and confidence within the dogs. Not only this, it makes the world a happy place to live in. Dogs in various attires wearing different attitudes walk on the ramp and attract the judges an people around them. Their way, their gait their charisma all are factors judges are interested in. Dogs do find their joys in this out-of-daily routine show.
					</div>
						<img src="img/about/14.jpg"  class="col-lg-5" style="border-radius: 40%;border-width: 10px; border-top-color: rgb(31, 69, 36); border-left-color: rgb(8, 8, 8); border-right-color: rgb(8, 8, 8); border-bottom-color: rgb(22, 69, 33); border-bottom-right-radius: inherit; border-top-left-radius: inherit; background-color: black; "/>
				</div>
			</div>
        </div>
    </section>
	
<?php include('footer.php'); ?>